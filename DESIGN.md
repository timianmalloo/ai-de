...
| **PS-R5** | **The rail follows the body; it never leads it.** The selection has one writer — the presenter's change event — so a destination can never read as selected while its body is not on screen; every other path a list selects on (a page key, a right-click, typeahead, the UIA `Select()`, a programmatic selection) is reverted and becomes an activation request; Tab enters the group on the selected destination and its container is never a one-stop (`TabNavigation=Once`) group. The window title reads *"<workspace> — <Perspective> — AI-DE"* and the status strip names *"<Perspective> perspective"* after every switch (spec §C6); the switch itself is one `shell.mode` line (from · to · trigger · `first_entry` · `outcome` · `error_code`) and, at the new body's first layout pass, one `shell.mode.shown` line with `duration_ms` (US-C12). |
...
| **PS-C3** | **A refusal marks the gap** inline, with one line of reason, names the fix, and never names a tier — the one content-gap refusal is tier-blind (Ruling 75): *"This prompt is a goal block and needs Not in scope."*; a blank Goal or Done when is never refused (the prompt is a Message, spoken at Send). A missing write scope says what derives one and offers the picker: *"Send needs a write scope. Mention the files or folders this run may write, as @path (for example @src/AiDe.Core/) — the lease is derived from your mentions."* **After any refusal Send is disabled with the reason beside it** until the gap is filled — one refusal grammar, not two. The rule that decides which tier a prompt compiles to is the operator's compile-step specification (not yet written); until it exists the refusal path's slice is gated on it. |
| **PS-C4** | **The page draws from the theme's tokens.** The host pushes, on `host.init`, exactly these eleven CSS custom properties from `Application.Resources` (`ComposerPageTheme.Roles`, merged with INV-0008): `--surface`, `--surface-raised`, `--surface-sunken`, `--text`, `--text-muted`, `--text-disabled`, `--accent`, `--accent-contrast`, `--border`, `--danger`, `--focus`; the stylesheet reads each with the token's declared value as its fallback for the pre-push frame and carries no second palette (TC5). A role the page needs beyond these is one additive row in `Roles`, never a literal — and **three are declared now, as rows of this contract** (the Shell lane's seam to the Conversation lane, ADR-0031's plan §Seams): **`--inferred`** → `{colors.inferred}` (`InferredBrush`) for the derived mark, **`--verified`** → `{colors.verified}` (`VerifiedBrush`) for the edited mark, **`--border-strong`** → `{colors.border-strong}` (`BorderStrongBrush`; App.xaml carries no brush of that key yet — the row lands with its brush) for the editor's boundary. Fourteen roles once the page reads them; the pinned-role test grows by exactly these three. The census (`EveryTextPairingInTheComposerPageClearsItsFloor`, `TheComposerPageDrawsWithTheTokensTheShellPushed`) walks the page's DOM. |
...
| `IconCoding` and `IconArchitecture` are resources of `MainWindow.xaml`, not of `App.xaml`'s registry (PS-R4) | The Shell lane's SH-2 slice does not own `App.xaml`; the two geometries are keyed resources resolved by the rail exactly as registry entries are, and move to the registry by a two-line seam request. Recorded so the split registry is a decision, not drift. |
| The drop-with-report sentence's second half reads *"<Perspective> admits them; open them from its View menu."* rather than spec §C4's *"Architecture opens with Graph, Domain, Contexts and Joins; open anything else from its View menu."* | The spec's sentence names §B4's Architecture default, which lands with SH-3 (`WorkbenchLayout.Default(perspective)`); until it does, naming the admitting perspective is the true sentence. The first half (count, captions, kinds, plural forms) is the spec's verbatim. |
| The rail's **opening** state (progress ring on the glyph, *"Opening Architecture…"*, focus held on the trigger) is not built | Both hosts are composed at construction and every body is retained, so a switch is synchronous and the state has no interval to show; focus stays on the trigger until the body's first layout pass by construction (`PerspectiveShell.EntryFocus` runs on `Loaded`). The row stays in the table for the day a body builds asynchronously; the spec's owner strikes or keeps it (US-C1's rationale for manual activation is unaffected). |
| The switch is two log lines (`shell.mode`, then `shell.mode.shown`), not US-C12's one | The switch line is written synchronously — INV-0009's readers and the headless presenter tests depend on it — and the stop edge (the body's first `Loaded`) is asynchronous and absent in a headless run. Two lines joined by `mode` and `trigger` keep the duration measured, never modelled, and never make the synchronous line wait. |

### Errata after Rulings 83–84 (2026-09-13, node D3): a fourth perspective, and Coding's default re-cut

*Node D3 (`docs/reviews/ui-operator-findings-2026-09-13.md`); the rows above are read as amended here; the mockup is `docs/mockups/perspective-shell.html`; no colour role is added.*

| Row above | Was | Now (the ruling, and D3's rendering of it) |
|---|---|---|
| Direction brief — *"three jobs"*, *"each perspective a spoke"* | Coding · Explore · Architecture; J1–J3 | **Four spokes.** Coordination is the bench for **J4: observe the fleet** — the Loomkeeper watcher's Terminal sessions, Message board, Leaderboard, Ledger and Daydreams, **read in parallel; no composer on that bench** (a dispute takes a reason; a session's turns are typed in Coding). `Arch:HubAndSpoke` is unchanged; the hub has one more spoke (Ruling 84). |
| The rail — composition row | New session; Coding · Explore · Architecture; Tests reserved | New session; Coding · Explore · Architecture · **Coordination** (`("coordination", "Coordination", 4, DockHost, "perspective.coordination")`, **Ctrl+4**, host C). Tests stays reserved and absent (AR3). The glyph is a new registry entry, **`IconCoordination`**: a ring with three nodes on it (a round table) on the `{icon.md}` grid at `{icon.stroke}` — round where Architecture's blocks are square and Explore's folder is a tab; the fleet as sessions in one circle. Tooltip *"Coordination — Ctrl+4"* rendered from the bound gesture (PS-R4); accessible name *"Coordination perspective"*. |
| PS-M1 — which menus each perspective has | Coding: all six · Explore: File · View · Help · Architecture: all but Prompt | **Coordination: File · Edit · View · Window · Help** (a host; Prompt is Coding's, spec §B3 rule 3). View → Perspective gains *Coordination Ctrl+4*. View's derived group **Coordination surfaces** reads *Show Terminal sessions · Show Message board · Show Leaderboard · Show Ledger · Show Daydreams*, then *Raise dispute on the latest scored episode* (a ledger verb follows the ledger). **Coding's View group loses those six rows** and keeps *New Search · New Code viewer · Show Diagnostics*. |
| The two default layouts and their empty states — Coding | Left = Terminal sessions · Center = empty · Bottom = one terminal · Right = empty (Ruling 60) | **Coding, re-cut (Rulings 83 and 88):** Left = **session documents** — the `session-document` zone rule is **Left** (the kind row's `Zone` column); a new session opens there **docked, `Maximized == null`**; the zone is empty until one opens and renders as its collapsed rail; `NewSessionPlacement` retired (SH-4.2) · Center = the empty state · Bottom = **one terminal, collapsed** — the operator's *Bottom (1)*; the terminal is one gesture away and is not started until the rail is expanded (`CollapsedBottomTests`) · Right = empty. **Coding's Left extent is 1.3 of the Center** (`WorkbenchLayout.CodingLeftExtent` = 1.3 ⁄ 2.3), **measured on the composed tree** (`CodingsLeftExtentTests`, SH-4.2): at 1440 × 900 the body is 1369 px, the Left pane 771, the document 751, the words' column 693 — the 96ch measure (672.6 px at 13 px) plus its margin fits, and the words wrap at the measure; at the largest window the measuring display gives — 1728 × 1117 DIPs, a 2560 × 1600 panel at 150 % scaling (the same panel class as the operator's; whether theirs runs at 150 % is an attended question) — the Left pane is 931 and the column 853; a 2560-DIP window (a 2560 × 1600 panel at 100 %) is not reachable here and is an attended row. `Ctrl+K, Z` (`workbench.maximizePane`) remains the on-demand whole tree. |
| The two default layouts — a third host | *(none)* | **Coordination (host C; Ruling 84's default, Inferred; D3 keeps the Owner's arrangement):** Left = Terminal sessions (the master list; the switch lands here) · Center = **Ledger · Leaderboard · Message board** as three tabs, Ledger first (its rows are the widest) · Right = empty · Bottom = collapsed · Daydreams from the View menu. The operator's screenshots 4 and 5 show the four read as one Center tab strip inside Coding — the same reading shape, now on its own bench. Slot file `<layout>.coordination.zones.json` (ADR-0032 rule 1). |
| The empty states — the Center's copy in Coding | *"No session open."* + **New session** | Two true sentences, by state (Ruling 83 condition 2): **no session in the layout** — *"No session open."* + **New session** (Ctrl+N) + *"Or open a recent one from File → Recent sessions."* · **a session open at Left** — *"The session is docked at the left."* + *"Code viewers, prompt drafts and search open here, from the View menu."* (true by the allow-list: `codeviewer`, `prompt`, `search` are Coding's Center kinds) — **no first action**: the first action is the editor at Left, where focus lands (the thread's own empty-state exception); a *Maximize* here would hide the zone that offers it and re-install the maximized state whose reconcile blindness is the open defect. The session pane's tab strip keeps its maximize / restore control (D2's design, Ctrl+K, Z). The Center never says *No session open* while one is open at Left. |
| The empty states — Coordination | *(none)* | `state.not-declared` shape, one per kind: Ledger *"No episodes scored yet."* / *"The ledger fills as observed sessions close episodes."* · Leaderboard *"No scores yet."* / *"Scores appear once an episode is scored."* · Message board *"No board posts yet."* (the product's string, kept) · Terminal sessions: the product's own (spec §A7) · Daydreams *"No daydreams yet."* / *"Patterns appear once the watcher has seen enough episodes."* No first action: these fill from observation, never from a click. |
| The switch, and what it says — the landing | Coding: the active document's editor, else the Left zone's active tab | Coding: the active session document's editor (**now in the Left zone**), else the Center empty state's first action · **Coordination: the Left zone's active tab (Terminal sessions)** — a reading host lands on its master list, as Architecture lands on Graph. Announcement *"Coordination perspective — 4 panes"*; title *"<workspace> — Coordination — AI-DE"*. |
| Copy — the drop-with-report | *"3 panes from your saved layout aren't available in Coding — Domain (class diagram), Graph, Contexts. …"* | Adds the form the Loomkeeper kinds take when a pre-C Coding envelope carries them (ADR-0032 rule 2; Ruling 84 — reported, **naming Coordination**, in spec §C4's form, with the saved caption and the kind title where they differ — `LayoutPersistence.Caption`): the operator's own 2026-09-13 layout reports *"4 panes from your saved layout aren't available in Coding — Ledger, Leaderboard, Sessions (terminal sessions), Board (message board). They live in Coordination (Ctrl+4); open them from its View menu."* (the kind title lower-cased in the parenthesis, as `LayoutPersistence.Caption` prints it — the review's P6 oracle, the form that shipped; *"opens with them"* asserted the contents of Coordination's default, which is Inferred and false for Daydreams, where *"live in"* is true by the allow-list for every admitted kind — DC-196, reconciled by SH-4.2); the pre-C default reports the same four by their registry captions. Singular: *"1 pane from your saved layout isn't available in Coding — Daydreams. It lives in Coordination (Ctrl+4); open it from its View menu."* **A session document saved in the Center is restored where it was saved** — the Left zone rule governs *opening*, never a restore *(Inferred; Ruling 83: "reopen is unchanged")*. This report class dwells until superseded, not for Ruling 86's 10 s — at first launch the operator is looking at the session, not the strip *(a finding for X-3)*. |
| Copy added by this section | — | `Coordination — Ctrl+4` · `Coordination perspective` · `Coordination perspective — 4 panes` · `Coordination surfaces` · `Nothing open here.` / `The session at the left could not be restored. Reopen it from File → Recent sessions, or close its tab.` *(the Center while the session fails to restore — names the way out the island offers; SH-4.2's UX review struck "its recovery is on the left", a recovery the island did not offer)* · `“<session>” could not be restored — its files are missing or unreadable. Reopen it from File → Recent sessions, or close this tab.` *(the island at Left for that session; never "No session is open" one pane over)* · `The session is docked at the left.` / `The sessions are docked at the left.` / `… at the left and the bottom.` *(the zone named from where the sessions are)* · `The session is docked at the left, collapsed — expand the rail to reach it.` *(the Left rail holding it)* · `Code viewers, prompt drafts and search open here, from the View menu.` · `Nothing open here.` / `Open a view from the View menu.` *(an empty Center in Architecture or Coordination)* · `New session` + the `Ctrl+N` chip *(the catalog's gesture, visible; the accessible name is the caption alone, the gesture its accelerator)* · `Console — open as a document in the Center` / `Opens the Console as a document in the Center; press again, or close its tab, to close it.` *(the session header's Console toggle, once hosted — its state is the document's open state)* · `Console open, <status>.` / `Console shown, <status>.` / `Console closed.` · `Session console` *(the View menu's row)* · `Maximize the session (Ctrl+K, Z)` *(the session pane's tab-strip control)* · `No episodes scored yet.` · `The ledger fills as observed sessions close episodes.` · `No scores yet.` · `Scores appear once an episode is scored.` · `No daydreams yet.` · `Patterns appear once the watcher has seen enough episodes.` |
| Recorded deviations | — | Coordination's default arrangement and Coding's Left extent are **Inferred** until the operator runs the next build (the attended rows in the review's §9). The reconcile's blindness under a maximized stack (`RefusedReconcileAnnouncement`) is a Shell-lane defect class the re-cut removes the trigger of, not the cause (Ruling 83's filing note). |
...
| Property | Value | Why |
|---|---|---|
| Chrome above the thread | **28px**, one row, and its content fits it (24px controls, no overflow) | Name · template · session settings · Console · the turn count (the jump list) · the budget state (*12,400 tokens this session · bounded by your subscription* / *38,900 of 40,000 · cap enforced* — per-session spend, derived, never stored; Ruling 78) · health. **No class chip**: the class is per turn (Ruling 70). |
...
| Turns at least half visible | **A per-viewport measured threshold (Ruling 88 condition 3; SH-4.2's L6, `CodingsLeftExtentTests`):** the session docked at Left, the Bottom collapsed, the editor at its 280 px rest. **At 1440 × 900: ≥ 1** — measured 1 of 2 realized with the review's fixture (b4 carries 77 tool calls: 2018 px, 0 shown; b5 345 px, 237 shown in a 237 px thread viewport) and 1 of 3 with five plain-prose turns (205 px each; b5 fully shown). **At 2560 × 1600: ≥ 1 measured, ≥ 2 Inferred** — the measuring display (a 2560 × 1600 panel at 150 % scaling, 1707 × 1067 DIPs) gives at most a 1728 × 1117 window, where the thread viewport is 421 px and the counts are 1 of 3 (the fixture) and 2 of 4 (prose); these ARE the operator's-panel numbers at 150 %, and a 2560-DIP window (100 % scaling) is an attended row. The ≥ 2 / ≥ 1 sub-cases (structure expanded, a refusal, a last turn that failed, waits or runs; the Console open — now a Center document, Ruling 89 — or *what was read* open) hold at ≥ 1. The row prints the viewport, the threshold and every realized turn's height and shown pixels. | Density: the tree shows a conversation, not one exchange — and a number that was measured, never the mockup's. |
...
- `Preparing… press again when prepared.` · `b1 is running. Wait — send after b1, or start a parallel session.` · `b5 is waiting for you. Wait — send after b5, or start a parallel session.` · `b2 queued — sends after b1` · `b1 stopped by you; b2 is waiting — Send it or cancel it` · `b2 is queued; cancel it or wait.` · `sent as the first turn of a parallel session` · `Refused before it started; a retry before 16:00 is refused again and spends nothing.` · `2 derived lines are not kept; they send empty.`
...
- `Spend has reached the cap you set (40,120 of 40,000 tokens this session): allow this turn, raise the cap, or stop. Nothing has run.` *(Ruling 78 — a measurement, never a prediction; the cap asks in-thread before a turn starts and never stops a running turn, whose outcome line reports the overrun)*
...
| Deviation | Reason |
|---|---|
| `Layout:StreamingThread` **adopted** for the session document; spec `spec-addendum-c-perspectives` §C1 and §B2 say *not adopted* (composer beside canvas) | The operator's verdict and the whole-real-estate ask; the section above listed reply-beside-input as its own unvalidated residual. **Filed: Ruling 74** — Addendum C §C1/§B2 amended; Addendum A §A2/§A6/R16 by errata note; the specs agree. |
| The Console is the reply side of each turn; a Console pane beside the composer is not rendered at rest | Ruling 21's split survives as an on-demand view of the same stream (SC1). **Filed: Ruling 74** (Addendum A §A2/§A6/R16 via `note-conductor-spec-errata-session-thread`; C §B2 amended). |
...
| The header's control reads **`template none` / `template change-order v2`**, not *Free-form* / *Template* (Addendum B `:181`, C §B2 "shape control") | *Free-form* is now the default task class (Ruling 72) and *shape* names the submission shape (Addendum A R15 b2: Message \| Goal-block) on every decoration line; a third meaning for either word is a vocabulary collision. The send row's shape badge of the section above is **deleted**: the decoration line's `<shape>` and `template` segments carry it 24px above. **Filed: E1** — Addendum B `:181`/`:186` by errata note, C §B2 `:971`/`:977`, US-C13 and the glossary amended: *shape* is Message \| Goal-block, *template* is `none \| id@version`, *class* is the task class. |
| Addendum D's *mode: mechanical-only* compile-line string is not rendered, and a tenth string, *Cancelled — draft edited*, is | Under `mechanical-only` with no template the compile line is absent (SC5: four rows at rest); the compile mode is a session setting the popover shows. An edit during *preparing* abandons an opened envelope, which Flow D-1 routes back to *draft* without a word; the design says so on the compile line. **Filed: E5** (the string is provenance, `opened.compile_mode`) and **E4** (`cancelled` stays the seventh outcome; *… reused, no new request* is the tenth string; Flow D-1's edge carries the cancelled string). |
| `template`, `suspect` and `gaps` refuse Send with *This prompt is a goal block and needs Not in scope.* — `gaps` carries exactly one gap; under `agentic-advisory` two unkept lines send blank with Send enabled, as Addendum D §A11 says | `SpawnContract.Validate` refuses a blank boundary tier-blind (C's own shape rule), so a goal block needs all three lines at any tier. **Ruled: Ruling 75** — the one true sentence, tier-blind; a blank Goal or Done when makes a Message (announced at Send), never a refusal; under advisory an unkept Goal or Done when → Message, an unkept Not in scope with both others confirmed → refused with *keep* offered; *they send empty* is true only for lines whose blankness yields a Message. |
| A Send while a turn runs or waits offers *"b1 is running. Wait — send after b1, or start a parallel session."* — a two-action status line, no dialog, no modifier key | **Ruled: Ruling 95**, reversing Ruling 77(b) as its condition 2 foresaw. *Wait* compiles now (the same gate, the same bytes, sha recorded) and queues **exactly one** turn — a thread row in state `queued` (*"b2 queued — sends after b1"*) whose only action is **Cancel** (its words return to the editor; editing is cancel-and-redraft). It sends automatically only when b1 ends Completed/Answered; after Stop or a failure it waits — *"b1 stopped by you; b2 is waiting — Send it or cancel it"* — with **Send now** on the row for that case only. A further Send while one is queued: *"b2 is queued; cancel it or wait."* *Start a parallel session* derives a sibling (same workspace, backends and config, `origin = parallel:<parent id>`, the name by Ruling 99's rule), docked in Coding's Left zone beside the parent (Ruling 83), the draft sent as its first turn through the sibling's own gate; the parent's draft is consumed only then. Measured first: two engine processes on one workspace run concurrently (Ruling 95 condition 1, `docs/proof/send-while-running.md`). Ruling 77(a) unchanged: no *Send now* during *preparing* — wait, or Cancel (*"Cancel — send without the model's lines"*). |
...
### Errata after Rulings 80–82 and 87 (2026-09-13, node D3): the editor's rest, the Console's grain, the conversation

*Node D3 (`docs/reviews/ui-operator-findings-2026-09-13.md`); the rows above are read as amended here; the mockup is `docs/mockups/session-conversation.html`; no colour role is added.*

**The comparable is the CLI's transcript idiom** — rendered prose, a collapsed *thinking* line, one line per tool call with a disclosure, then the answer *(Inferred from use)*; what screenshot 3 showed instead, counted, is the review's §2b. The rows below make the thread render the fold it already holds.

| Row above | Was | Now (the ruling, and D3's rendering of it) |
|---|---|---|
| Direction brief — constraints, *"the maximized Center stack (Ruling 47)"* | the session takes the whole tree | The session document is **docked in Coding's Left zone** (Ruling 83; the shell section's errata); `Ctrl+K, Z` is the on-demand whole tree. The contract below is measured at 1440 × 900 with the session **docked at Left, extent 1.3** — the 96ch measure must still fit (Ruling 83 condition 1). |
| *"Not chunky"* — editor share (the front-door row this section inherits) | ≥ 45 % of the zone height; never below 130px | **Derived from the thread's need (Ruling 80).** **0 turns:** the thread is its two-line caption and the editor takes the rest of the body — no vertical scrollbar before the operator types. **≥ 1 turn:** the editor **rests at 280px** and scrolls only past it; the thread takes the remainder. **130px is the floor** under a small window — **one constant**, `ComposerSurface.EditorFloor`, read by the host and pushed to the page (`composer.html`'s own `min-height: 110px` is a second definition of one floor, DM-A: the page reads the pushed value). `SessionDocumentSurface.ComposerShare` retires. |
| "Chat-like" — the editor's top edge | equal at 1, 5 and 40 turns, read in one pass | Equal at 1, 5 and 40 turns; **at 0 turns the editor's bottom edge is the composer's and its height is `body − header − caption − chrome`**; one audit pass reads all four. |
| "Chat-like" — editor height | ≥ 130px, ≤ 280px then scrolls | **280px at rest with turns** (≥ 130 only when the window cannot give 280); **fills the body at 0 turns**; a scrollbar appears only when the text exceeds the rest height. |
| The turn — reply side: outcome line | first on the reply side; *N events* folds every event | **Last on the reply side** — the conversation ends with its outcome (Ruling 82's order: prose · reasoning · tool call+result · outcome). *N events* folds **only the non-conversation rows** (`acp.*`, the conductor's accept / complete lines, stderr): Ruling 74's one fold, re-pointed to `Coalesce` (Ruling 81). The failure box and its actions follow the outcome line. |
| The turn — reply side: the reply | plain text, one blob | **The conversation, in event order**, rendered from **`Coalesce(turn.Events)`** — the same pure function the Console split reads (Ruling 81; one derivation, two readers, DM7): **prose** — `agent.msg` coalesced, **rendered markdown** (headings 13px `{typography.weight-medium}` — one size, weight carries the level; lists; `{typography.mono}` code on `{colors.surface-sunken}`; tables by hairline, never a box) with **no link activation** (a link is its text followed by its URL, visible, in `{typography.mono}` `{colors.text-muted}` — never a control, never hover-only) and **UTF-8 throughout** — `—` and `§` render as themselves (Ruling 87) · **reasoning** — `agent.thought` coalesced · **tool call + result** — one item per `tool.call`, its `tool.result` rows attached by `toolCallId` · the outcome line. `acp.*` frames (`usage_update`, `available_commands_update`, `acp.result`) are **Console evidence, never conversation**; usage feeds Spend (Ruling 78). |
| The turn — reply side: events | 20px lines; folded on a completed turn; the running turn shows its last four live | **Tool item** — a 24px line inline: one neutral decorative marker (the CLI's bullet) · **the kind as a muted word from the frame's `kind`** (`read` · `edit` · `execute` · `search` — text, never a per-kind icon: 1.1.1) · the title in `{typography.mono}` `{colors.text}` · the status word (the ring *running*, `{colors.verified}` *done*, `{colors.danger}` *failed*, `{colors.text-muted}` *interrupted*) · **detail ▸** — a disclosure that opens the input and the result as 12px `{typography.mono}` on `{colors.surface-sunken}`, ≤ 200px then scrolls. **Reasoning item** — the CLI's collapsed-thinking idiom (the Owner's filing note, D3's call within Ruling 82's bounds): one 24px line *Thinking* in `{colors.text-muted}` beside a three-dot glyph (no duration: the fold carries the first chunk's timestamp only, and time-to-the-next-event is a plausible wrong number), a disclosure that opens the coalesced thought as 12px `{colors.text-muted}` **plain text** at 96ch (no markdown, no links — it is not the lane's answer); **collapsed by default, never announced** (SC9), `aria-live="off"`, no per-session "always open" setting (a next step, not scope). The **running** turn renders its items live, the last one growing; **Stop this turn** stays on the outcome line. Non-conversation rows keep the 20px event-line form inside *N events*. A `tool.call` with **no result on a turn that is no longer live** (stopped, exited) reads ***interrupted*** in `{colors.text-muted}`, static — never *running* with a ring. A failed **past** turn renders its conversation like any other; only its actions fold (SC7). |
| SC7 | outcome line, reply, *N events* collapsed | A completed turn: the conversation's items, then the outcome line carrying *N events* (non-conversation rows only); the failure box and its actions after it. |
| SC8 | inside a turn, Tab reaches provenance · compiled prompt · events · the turn's actions | … provenance · compiled prompt · **each reasoning disclosure · each tool item's detail** · events · the turn's actions, in DOM order — **except on a running, waiting, failed or stopped last turn, whose entry stop is its first action** (*Stop this turn* · *Deny* …), and **Shift+Tab from the editor lands on that action** (the composer follows the thread). An opened detail or compiled prompt is a keyboard-scrollable region (a stop of its own while open; PageDown inside it scrolls it, never jumps turns). A status change **updates the item in place** — focus inside a turn survives every snapshot (DS-1 Q10's rule, restated for items). |
| SC9 | event lines are never announced | … **reasoning and tool items are never announced** (they are event-derived; the outcome is announced once); prose is not announced while it streams. |
| SC1 — the Console split | a view of the same stream unfolded in time, one line per event, beside the thread | **With the session docked at Left the split opens in the Center zone** — a *Console — <session>* document beside the session, the natural occupant of *"code viewers, prompt drafts and search open here"* — never inside the Left pane, whose 673px cannot hold a thread and a Console side by side *(Inferred: Ruling 21/74 say "on demand", not where; a question for the conductor)*. **One row per message (Ruling 81):** `hh:mm:ss · lane · message · <the joined text> · n chunks`; a **thought** row the same, dim; `tool.call` · `tool.result` · `permission.request` · `acp.*` stay **one row per event**. A message interrupted by a tool call is two rows — the boundary is the interleaving. The chunk count is rendered from the fold, never asserted. **The split's rows equal `Coalesce(turn.Events)` of every turn, in order, and the thread's reply side renders the same `Coalesce` output** (Ruling 74 condition 1 as amended). |
| "Chat-like" — new rows | — | *Console rows = `Coalesce(events)` of every turn, in order* (an identity read from the rendered DOM) · *Reasoning rows announced: 0* · *0 turns: no vertical scrollbar in the editor; editor height = body − header − caption − chrome* · *the editor rests at 280 at 1 / 5 / 40 turns with the structure collapsed* · *geometry (document · composer · thread · editor), reported*. The **≥ 3 turns half visible** row is read at 1440 × 900 and at the operator's 2560 × 1600 — the review's §2c carries what each layout gives. |
