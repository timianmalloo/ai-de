...
| The two default layouts and their empty states — Coding | Left = Terminal sessions · Center = empty · Bottom = one terminal · Right = empty (Ruling 60) | **Coding, re-cut (Ruling 83):** Left = **session documents** — the `session-document` zone rule is **Left**; a new session opens there **docked, `Maximized == null`**; the zone is empty until one opens and renders as its collapsed rail; `NewSessionPlacement` retires · Center = the empty state · Bottom = one terminal · Right = empty. **Coding's Left `DefaultExtent` is 1.3 of the Center** *(Inferred; the mockup measures it: at 1440 × 900 the Left zone holds the thread's 96ch measure plus its 40px gutter and 32px padding without clipping — Ruling 83 condition 1)*. `Ctrl+K, Z` (`workbench.maximizePane`) remains the on-demand whole tree. |
