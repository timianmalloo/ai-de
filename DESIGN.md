...
| Copy added by this section | — | `Coordination — Ctrl+4` · `Coordination perspective` · `Coordination perspective — 4 panes` · `Coordination surfaces` · `Nothing open here.` / `The session at the left could not be restored; its recovery is on the left.` *(the Center while the session fails to restore)* · `The session is docked at the left.` · `Code viewers, prompt drafts and search open here, from the View menu.` · `Maximize the session (Ctrl+K, Z)` *(the session pane's tab-strip control)* · `No episodes scored yet.` · `The ledger fills as observed sessions close episodes.` · `No scores yet.` · `Scores appear once an episode is scored.` · `No daydreams yet.` · `Patterns appear once the watcher has seen enough episodes.` |
