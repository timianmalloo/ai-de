...
*Applied by `/ui-design` (elevate) at node D3 over the operator's five 2026-09-13 findings
(`docs/reviews/ui-operator-findings-2026-09-13.md`); the rows above are read **as amended here**. The
mockup is `docs/mockups/perspective-shell.html` (the Coordination bench, the Coding re-cut). No colour
role is added: the fourth rail item and its bench use the pairs the matrix already measures.*

| Row above | Was | Now (the ruling, and D3's rendering of it) |
|---|---|---|
| Direction brief — *"three jobs"*, *"each perspective a spoke"* | Coding · Explore · Architecture; J1–J3 | **Four spokes.** Coordination is the bench for **J4: observe the fleet** — the Loomkeeper watcher's Terminal sessions, Message board, Leaderboard, Ledger and Daydreams, **read in parallel and entered nowhere** (nothing on that bench takes typed input; the composer stays in Coding). `Arch:HubAndSpoke` is unchanged; the hub has one more spoke (Ruling 84). |
| The rail — composition row | New session; Coding · Explore · Architecture; Tests reserved | New session; Coding · Explore · Architecture · **Coordination** (`("coordination", "Coordination", 4, DockHost, "perspective.coordination")`, **Ctrl+4**, host C). Tests stays reserved and absent (AR3). The glyph is a new registry entry, **`IconCoordination`**: three nodes joined by two strokes (two above, one below) on the `{icon.md}` grid at `{icon.stroke}` — the fleet as a graph of sessions, distinct from Explore's folder and Architecture's blocks. Tooltip *"Coordination — Ctrl+4"* rendered from the bound gesture (PS-R4); accessible name *"Coordination perspective"*. |
...
| The empty states — the Center's copy in Coding | *"No session open."* + **New session** | Two true sentences, by state (Ruling 83 condition 2): **no session in the layout** — *"No session open."* + **New session** (Ctrl+N) + *"Or open a recent one from File → Recent sessions."* · **a session open at Left** — *"The session is docked at the left."* + **Maximize the session** (Ctrl+K, Z) + *"Code viewers, prompt drafts and search open here."* (true by the allow-list: `codeviewer`, `prompt`, `search` are Coding's Center kinds). The Center never says *No session open* while one is open at Left. |
...
| Copy — the drop-with-report | *"3 panes from your saved layout aren't available in Coding — Domain (class diagram), Graph, Contexts. …"* | Adds the form the five Loomkeeper kinds take when a pre-C Coding envelope carries them (ADR-0032 rule 2; Ruling 84 — reported, **naming Coordination**): *"5 panes from your saved layout aren't available in Coding — Terminal sessions, Message board, Leaderboard, Ledger, Daydreams. They live in Coordination (Ctrl+4); open them from its View menu."* Singular: *"1 pane from your saved layout isn't available in Coding — Ledger. It lives in Coordination (Ctrl+4)."* |
| Copy added by this section | — | `Coordination — Ctrl+4` · `Coordination perspective` · `Coordination perspective — 4 panes` · `Coordination surfaces` · `The session is docked at the left.` · `Maximize the session` · `Code viewers, prompt drafts and search open here.` · `No episodes scored yet.` · `The ledger fills as observed sessions close episodes.` · `No scores yet.` · `Scores appear once an episode is scored.` · `No daydreams yet.` · `Patterns appear once the watcher has seen enough episodes.` |
...
*Applied by `/ui-design` (elevate) at node D3 over the operator's five 2026-09-13 findings
(`docs/reviews/ui-operator-findings-2026-09-13.md`); the rows above are read **as amended here**. The
mockup is `docs/mockups/session-conversation.html`. No colour role is added: the reasoning item and
the tool item use `{colors.text-muted}`, `{colors.verified}`, `{colors.danger}` and the mono face on
the pairs the matrix already measures.*

**What the CLI shows that the thread did not.** The Claude Code transcript the operator names
*(Inferred from use; not fetched)* renders, per turn and in order: the assistant's prose as it
streams, **rendered** (headings, lists, code, tables); a **collapsed *thinking* line** the reader
may open; each **tool call as one line** with its title and status and a **disclosure** for its
input and result; then the answer. Screenshot 3 (`the output should show the conversation and
reasoning…png`) shows the thread rendering the reply as one plain-text blob of markdown source
(`## Gaps I noticed`, `|---|---|`) with the tool calls only in the *N events* fold and, in the
split, one row per wire chunk (`mer` · `ges are` · `missing from the tracker and`) and the mojibake
`â€"` / `Â§` (Ruling 87). The rows below make the thread render the fold it already holds.
...
| The turn — reply side: events | 20px lines; folded on a completed turn; the running turn shows its last four live | **Tool item** — a 24px line inline: a kind glyph (`execute` · `edit` · `read` · `search`, from the frame's `kind`) · the title in `{typography.mono}` `{colors.text}` · the status word (`{colors.text-muted}` *pending*, the ring *running*, `{colors.verified}` *done*, `{colors.danger}` *failed*) · **detail ▸** — a disclosure that opens the input and the result as 12px `{typography.mono}` on `{colors.surface-sunken}`, ≤ 200px then scrolls. **Reasoning item** — the CLI's collapsed-thinking idiom (the Owner's filing note, D3's call within Ruling 82's bounds): one 24px line *Thinking · 4 s* in `{colors.text-muted}` beside a three-dot glyph, a disclosure that opens the coalesced thought as 12px `{colors.text-muted}` **plain text** at 96ch (no markdown, no links — it is not the lane's answer); **collapsed by default, never announced** (SC9), `aria-live="off"`, no per-session "always open" setting (a next step, not scope). The **running** turn renders its items live, the last one growing; **Stop this turn** stays on the outcome line. Non-conversation rows keep the 20px event-line form inside *N events*. **Density rule (Inferred — D3's, not in Ruling 82's text; measured in the mockup as its own harness axis and put to the conductor):** on a **completed** turn a run of **≥ 4 consecutive tool items** collapses to one 24px group line — *12 tool calls · 3 edits · 9 reads · ▸* — that expands in place; the running turn never groups. |
...
| SC1 — the Console split | a view of the same stream unfolded in time, one line per event | **One row per message (Ruling 81):** `hh:mm:ss · lane · message · <the joined text> · n chunks`; a **thought** row the same, dim; `tool.call` · `tool.result` · `permission.request` · `acp.*` stay **one row per event**. A message interrupted by a tool call is two rows — the boundary is the interleaving. The chunk count is rendered from the fold, never asserted. **The split's rows equal `Coalesce(turn.Events)` of every turn, in order, and the thread's reply side renders the same `Coalesce` output** (Ruling 74 condition 1 as amended). |
| "Chat-like" — new rows | — | *Console rows = `Coalesce(events)` of every turn, in order* (an identity read from the rendered DOM) · *Reasoning rows announced: 0* · *0 turns: no vertical scrollbar in the editor; editor height = body − header − caption − chrome* · the **≥ 3 turns half visible** row is measured **twice**, with tool items inline and with runs grouped; the mockup prints both. |
...
| Copy added by this section | — | `Thinking` · `Thinking · 4 s` · `detail` · `pending` · `running` · `done` · `failed` · `12 tool calls · 3 edits · 9 reads` · `41 chunks` · `message` · `thought` · `Console: every wire frame, one row per message` *(the split toggle's description)* |
| Recorded deviations | — | The `agent.thought` row's shape is **Inferred** until a frame is captured (Ruling 82 condition 1 — the corpus has none). The grouping of tool runs on completed turns is **D3's density rule**, not Ruling 82's text: a question for the conductor with the measured numbers beside it. The reply's markdown renderer is the WPF thread's — which subset it renders (headings, lists, code, tables) is CV-5's design-slice question; the mockup renders that subset. |
