...
| A Send while a turn runs or waits is refused with *"b1 is running; the next turn waits for it."* | **Ruled: Ruling 77** — one governed run at a time per session in Phase 1, an Owner extension, marked, reversible: parallel runs would re-read it as *"queued after b1"* with a `queued` outcome. No *Send now* during *preparing*: wait, or Cancel (*"Cancel — send without the model's lines"*). |
