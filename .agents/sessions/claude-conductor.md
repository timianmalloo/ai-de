...
`docs/notes/attended-rows-for-the-operator.md` §2026-09-14 (E-1…E-3, C-1…C-5, S-1…S-7).

## Waiting on

| From | What |
|---|---|
| **`composer-r95`** | its close → join (full recount) |
| **`engine-spikes`** | its record → the accounts slice's brief and the per-engine slices |
| **The user (later)** | the attended rows; Ruling 88's density unit; a live acceptance of first use on their second machine (Ruling 104 condition 4) |
