...
const fs = require("fs");
const RECV_CAPTURE = __dirname + "/frames/read.jsonl";
const SENT_CAPTURE = __dirname + "/frames/read.sent.jsonl";
...
p.stdout.on("data", d => {
  ...
  while ((i = buf.indexOf("\n")) >= 0) {
    ...
    if (m.method === "session/update") {
      ...
      send({jsonrpc:"2.0",id:2,method:"session/new",params:{cwd:"C:/projects/ai-de",mcpServers:[]}});
    } else if (m.id === 2) {
      ...
      send({jsonrpc:"2.0",id:3,method:"session/prompt",params:{sessionId:sid,prompt:[{type:"text",text:"Run the shell command `git status --short` in the current directory and tell me the output. Do nothing else."}]}});
    } else if (m.id === 3) {
