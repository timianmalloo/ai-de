...
/// <summary>
/// The class-diagram surface (ADR-0026 class-diagram-architecture): renders a hierarchy built from graph nodes/edges. Host-side
...
public sealed class ClassDiagramSurfaceTests
{
    ...
    public void ShowGraph_DoesNotClaimMembersAreUnextracted()
    {
        OnSta(() =>
        {
            var s = new ClassDiagramSurface();
            s.ShowGraph(new[] { N("A", "class"), N("B", "class") }, new[] { Edge("A", "B", "inherits") });
            // The stale ADR-0026 class-diagram-architecture Phase 1 note ("Members are not extracted yet") is gone — members are extracted.
