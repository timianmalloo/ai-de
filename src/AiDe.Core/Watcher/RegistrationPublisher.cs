...
public static class RegistrationPublisher
{
    ...
    internal static void PublishNative(NativeNoticeDelivery notice, byte[] latestBytes)
    {
        ...
        NativeAdmissionCodec.ValidateCompatibilitySession(notice.SessionId);
        ...
        WithPinnedRegistration(notice.Root, create: true, () =>
        {
            RequireCompatibilityOwner(latest, notice.SessionId);
            var registration = Path.Combine(notice.Root, DirectoryName);
            var history = Path.Combine(registration, "notices");
            Directory.CreateDirectory(history);
            using var historyPin = OpenNativePath(history, directory: true);
    ...
    internal static void ValidateCompatibilityOwner(string root, string sessionId) =>
        WithPinnedRegistration(root, create: false, () =>
            RequireCompatibilityOwner(Path.Combine(root, DirectoryName, StandingPublisher.FileNameFor(sessionId)), sessionId));

    private static void WithPinnedRegistration(string root, bool create, Action publish)
    {
        ...
        lock (PublicationGate)
        {
            ...
            try
            {
                ...
                for (var directory = new DirectoryInfo(root); null != directory; directory = directory.Parent)
                    ancestors.Push(directory.FullName);
                foreach (var ancestor in ancestors)
                {
                    if (create && !Directory.Exists(ancestor)) Directory.CreateDirectory(ancestor);
                    pins.Add(OpenNativePath(ancestor, directory: true));
                }
                var registration = Path.Combine(root, DirectoryName);
                if (!create && !Directory.Exists(registration)) return;
                if (create) Directory.CreateDirectory(registration);
            ...
            catch (ArgumentException) { throw NativeAdmissionErrors.Error(NativeAdmissionErrors.ContextMismatch); }
            finally { foreach (var pin in pins) pin.Dispose(); }
        }
    }

    private static void RequireCompatibilityOwner(string path, string sessionId)
    {
    ...
    internal static void RequireCompatibilityOwner(Stream stream, string sessionId)
    {
        ...
        try
        {
            ...
            if (document.RootElement.ValueKind != JsonValueKind.Object
                || !document.RootElement.TryGetProperty("sessionId", out var owner)
                || owner.ValueKind != JsonValueKind.String || owner.GetString() != sessionId)
                throw NativeAdmissionErrors.Error(NativeAdmissionErrors.NoticeConflict);
        }
        catch (JsonException) { throw NativeAdmissionErrors.Error(NativeAdmissionErrors.NoticeConflict); }
    }
    ...
    public static string Publish(string coordLogDirectory, RegistrationNotice notice)
    {
        ...
        NativeAdmissionCodec.Text(notice.SessionId, 512);
        NativeAdmissionCodec.ValidateLocalPath(coordLogDirectory);
        var path = Path.Combine(coordLogDirectory, DirectoryName, StandingPublisher.FileNameFor(notice.SessionId));
        NativeAdmissionCodec.ValidateLocalPath(path);
        ...
        WithPinnedRegistration(coordLogDirectory, create: true, () =>
        {
            ...
            InstallNativeBytes(path, bytes, immutable: false);
            RequireNativeBytes(path, bytes);
        });
