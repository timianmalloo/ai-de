...
public sealed class WorkbenchShell : IDisposable
{
    ...
    /// (ADR-0026 class-diagram-architecture). Reads `_queries` live and wires the same context lookup the canvas uses, so a class
    ...
    // query (ADR-0026 class-diagram-architecture Phase 2). maxNeighbors is 1 because members ride on the node itself, not its edges.
    ...
    /// <summary>
    /// Feeds each open code-viewer surface content (ADR-0018 node-content-reader-contract / ADR-0025 code-viewer-renderer). Until Core's NodeContentAsync ships,
