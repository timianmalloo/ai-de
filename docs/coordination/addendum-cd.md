...
| 6 | join each as it closes (serially, in the primary): merge → `verify-test-run.py --update` (whole + both Core halves) → audit append → `regenerate-derived.py` → **commit** → `python tools/run-verify-gates.py && git push` (the runner is the one status; a gate set run before the commit is red on the primary's own uncommitted audit lines — DC-113 rec. 3); DC ids allocated | conductor ≈ 10 calls per join | The join rule; partial is acceptable - a failed node reports and its lane pauses, the others continue. |
