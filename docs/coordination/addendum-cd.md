...
| every track -> conductor | A new defect class: append the entry with the id left as `DC-nnn` and a `**Status:**`; the conductor allocates the number at the join (contiguous family; two live trees hold the two ids after DC-147). A `tools/expected-test-counts.json` bump: never edit; the conductor runs `verify-test-run.py --update` at the join. A `*.csproj` change: request. | conductor |
