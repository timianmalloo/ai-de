...
**Landed for F:** the #4 extent floor, and the **#11/#3-focus focus-preservation** — a verified
model→view desync (AvalonDock reset each pane's selection on attach and lost active-content on the
wholesale layout swap), fixed by `RestoreSelection` (re-apply the model's active tab per pane) and
`RestoreActive` (re-focus the pre-swap surface, but only when the model still agrees it is active, so
a user activation still wins), with 2 adapter tests. **Remaining:** #10 (native-drag kind-fallback
relocating the graph) and #12 (confirm) — reconcile concerns that need a functional WPF drag repro to
...
| **F ◐** | Dock drag/close/focus (§5a). **#4 fixed** (empty tool zones floor to a usable width). **#11/#3-focus fixed** — `RestoreSelection`/`RestoreActive` preserve the model's active tab and the user's focus across `Render()`'s layout swap (2 adapter tests). #10 (native-drag kind-fallback) + #12 (confirm) remain — need a WPF drag repro | #4, #11, #3-focus (fixed); #10, #12 (designed) | App+Core | landed the tested fixes |
...
| **Remaining** | F's **#10** (native-drag kind-fallback relocating the graph) and **#12** (confirm restore-on-open) — need a WPF drag repro to fix without guessing; #9 provenance legibility (**inspected: the provenance rows already use full `TextBrush`, not opacity — the inferred Phase-B cause is absent; needs the actual snapshot to diagnose**) |
| **Best next action** | A functional WPF drag repro of #10 (native tab drag between zones), then resolve the `TryMapByPosition` extra-column fallback |
| **Needs user functional verification** | The **rendered** sequence diagram (E), class-diagram pan/zoom + right-click menus (C/D), terminal glyph refresh in Claude Code (G), the sessions chip legibility (H), and the **focus/tab-preservation on open/close** (F #11/#3-focus) — all beyond headless testing |
