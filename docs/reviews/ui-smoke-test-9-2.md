...
| 3 | "window system still goofy i moved joins from right to left and contexts now moved from left to right in the same move operation" | Docking | **I** — a native AvalonDock drag whose reconcile shape the content-anchored `TryMapByPosition` still can't resolve confidently falls back to kind-based `TreeToZones.Convert`, which re-seats **every** stack by kind — so moving `joins` (a tool pane) re-classifies `contexts` too. The #10 fix reduced but did not eliminate the fallback's blast radius | S2 | ▢ planned (T-W2) |
...
| **T-T2** | More keys: **F1–F12** (SS3/CSI), **Shift+Tab** (`ESC [ Z`), **Alt+key** (meta = `ESC`-prefix), **Ctrl+arrows / Shift+arrows** (modified CSI `1;<mod>`), Ctrl+Space (NUL) | Full hotkey/keystroke fidelity | App |
| **T-T3** | **Bracketed paste** (`ESC [ ? 2004 h`): parser tracks the mode; paste wraps text in `ESC [ 200~ … ESC [ 201~`; wire WPF paste (Ctrl+V / middle-click) | Multi-line paste into Claude Code without each line executing | App+Core |
