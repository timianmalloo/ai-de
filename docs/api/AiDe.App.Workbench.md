...
summary: >-
  Extracted public surface of AiDe.App.Workbench: 101 types, 443 members, 74% carrying a summary doc comment.
---

# API: `AiDe.App.Workbench`

**101 public types · 443 public members · 74% documented.**
...
honest 0 from a starved one.

### `void WebSurfaceHandshake(`

Records one transition of a web surface's host↔page handshake, with the surface's counts as
they stood at that moment.

**Remarks.** **The transitions** — from the host: `initialising`, `re-attached`,
`init-failed`; from the composer: `navigation-started`, `configured`,
`page-ready`, `init-pushed`, `input-received` (once per pushed init, at the
first accepted `draft.changed`), `message-dropped` (once per kind and reason per
navigation), `disposed`. Counts are lifetime totals as they stood at the transition;
