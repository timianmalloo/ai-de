...
| 4 | The command is wired into palette + menu (no drift) | `MainMenuTests.TheMenuCoversEveryCatalogCommand` + `Phase3SurfacingTests.DeclaredMenusMatchWhatTheBuilderRenders` | catalog/menu drift (DC-065) | seen (both reded before the builder + count were updated) | Verified | menu is a hand-maintained parallel list (DC-065) |
...
- `verify-defect-register.py`: OK (DC-065 added — the menu-drift class the conformance tests already control).
