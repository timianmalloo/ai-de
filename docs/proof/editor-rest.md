...
## Gates (at close, on `origin/main` `1c29b5d5` unchanged — nothing to merge; never rebased)

| Gate | Result |
|---|---|
| `dotnet build` Core · App · Core.Tests · App.Tests `-p:TreatWarningsAsErrors=true` | 4 × Build succeeded, 0 warnings, 0 errors |
| `tools/verify-test-run.py` (both suites, trx under `artifacts/test-results`) | App **913** executed (baseline 895; +18 this track) · Core **2591** (baseline 2591), both Completed, 0 failed, 1 expected skip |
| The full App run before the reviews' conditions | 909 passed, 0 failed (3 m 27 s); Core 2591 passed (1 m 59 s) |
| `tools/run-verify-gates.py` | 35 of 35 OK once the audit entry is committed (before the close-out commit: 34, `verify-stranded-audit` naming the uncommitted append-only log — this commit) |
| `tools/verify-ui-craft-floor.py` | OK — `src/AiDe.App/Web` 0 findings; every gated mockup clean at its threshold |
| `tools/regenerate-derived.py` | every derived view current; 188 classes verified (the three placeholders await the conductor's numbers); the docs index carries `proof-editor-rest`; one pre-existing finding (`front-door-ruling-49`'s dangling `proof-conductor-front-door`) |
| The audit entry | `al-01M2ECPT56TVX0NT3042TNTBCS`, `duration_seconds` 3147 measured from the start marker; signals acceptance-met · verification-path · verification-executed all true; its summary's "916" was written before the trx run — the record is 913 (a correcting entry follows it) |
| Release build `ProductVersion` | reported in the closing report (`dotnet build src/AiDe.App/AiDe.App.csproj -c Release`) |
| Commits | `5c0710a6` (the slice, the oracles, the reviews' conditions, the register, this pack) · the close-out commit (the audit entry, the derived views, this table) |
...
- **DC-191 (CV-5-4 a)** — a ceiling mistaken for a height: a belt (a maximum the composer composes
...
- **DC-192 (CV-5-4 b)** — a fill computed on the wrong box: `min-height: 100%` on one child of the
...
- **DC-193 (CV-5-4 c)** — `IsVisible` is false for every element of a detached WPF tree, so a
