...
| `tools/regenerate-derived.py` after the audit entry `al-01M2BT3TRC7RR7F6GGZ6GPF2HD` (7,064 s measured) | every derived view current; 14 site figures verified; the register 171 classes + five placeholders (`DC-nnn (CV-2 a–e)`, the conductor allocates) |
| Release build `ProductVersion` | `1.0.0+ed4f51bb330fd3f9e616e43801ef0891aecc6ded` (`dotnet build src/AiDe.App/AiDe.App.csproj -c Release`, 0 errors) |
| the run's terminal ledger (`%TEMP%ide-tests	erminal-ledger-<pid>.log`, read at the join) | 56 starts · 54 stops · 0 start failures on this run's App suite — the same 56/54 an earlier ledger on this machine shows (`terminal-1`, `s-terminal`: surfaces this slice does not compose); no `WorkbenchShell` is constructed by any test this slice adds |
