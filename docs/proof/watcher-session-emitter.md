...
summary: >-
  ...
  its shell wiring, including the DC-067 session-end-to-Ended fix. 9 emitter tests, 2 mutation-verified;
...
| 7 | **End→pump marks the session Ended (DC-067)** | `End_WritesSessionEnd_AndStopsTracking` | session-end not marking the store | **mutation-verified** (neutralise `EndSession` → red) | Verified | — |
...
- `verify-defect-register.py`: OK (DC-067 added; header count controlled 20).
