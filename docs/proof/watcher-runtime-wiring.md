...
# Proof Pack: Watcher wired into the running app (DC-066)
...
- **E11 (prove through the real composition root):** the regression exercises `AttachWorkspace` → `Adapter.Render` → `ContentFor("sessions")` — the exact runtime path — rather than constructing the factory directly (which is what hid the defect). This is the control for DC-066.
