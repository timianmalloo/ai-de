...
| Keyboard traversal works | Pointer required for View source / Reveal / expand | Enter View source; Ctrl+Enter Reveal; arrows; unindexed does not expand | Spike F8/F11/F13. UV-1 `HandleKey` Enter/Control; file double-click View source; node menu Reveal. View-source fault overlay E2E (`ViewSourceFault_ShowsCouldNotOpenSource_…`). Physical Ctrl+Enter **Flagged** (SendInput is DC-014 / testhost foreground; RaiseEvent is not the chord) | yes (Enter, menu, double-click, source-fault overlay) | **Verified** (Enter, arrows, menu, double-click, source-fault Retry) / **Flagged** (physical Ctrl+Enter) | Attended SendInput still owed |
