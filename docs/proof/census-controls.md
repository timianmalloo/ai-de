...
summary: >-
  ...
  (DC-158) surfaced by the reach and routed to the Shell lane, not fixed here (App.xaml is outside
...
### A finding the reach surfaced (DC-158, routed to the Shell lane)
...
1. The gesture-chord `TextBlock` in `MenuItemSubmenuItem` sets `Foreground="{DynamicResource
   ...
   pinned by name in `TheDC158MenuInkGapIsNamedAndDoesNotWiden` so a third site cannot silently
   join it.
...
Registered as **DC-158** in `docs/lessons/defect-classes.md` (id left as issued on this branch;
`verify-id-allocators.py` already reports it colliding with `origin/main`'s own DC-158 — expected
...
> DC-158 — add the missing `IsEnabled=False` trigger to `MenuItemTopLevelHeader` (mirror the other
...
**Also observed, not caused by this track:** `verify-id-allocators.py` reports DC-158 colliding
with `origin/main` (another track merged its own DC-158 first). Per the session contract
...
updated to `controlled`; DC-158 registered `partially-controlled`) → `tools/verify-mockup-audits.py`
