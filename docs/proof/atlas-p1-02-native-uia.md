...
# Per-view loaded/visible correction receipt — 2026-09-16

**Author correction frozen for independent review.** The settled field table above requires each retained `AtlasReaderView` row to report its public `IsLoaded` and `IsVisible` values. The reviewed source omitted both fields from `ViewObservation`, so host/window flags could not substitute for the detached view's own state. This unit changes only the test-local observer, its persisted adapter oracle, and this proof.

`ViewObservation` now carries nullable `IsLoaded` and `IsVisible` fields. The owning-STA sampler reads each public scalar independently. A successful read records the actual Boolean. A read failure leaves only that field null and adds the bounded unavailable reason; it does not invent a value, retry, perform layout/focus/refresh, or activate a window. The existing record construction, immutable receipt admission, three publication paths, original queries/assertions/timeouts, and all other limits remain unchanged.

Red-first evidence used the actual unshown owning-STA adapter and persisted `observer.wpf` JSON. Before the observer fields existed, `NativeObserver_NonGui_ActualAdapterDistinguishesHostsChildrenAndRetainedViews` failed on the required `IsLoaded` lookup with `KeyNotFoundException`: 1 executed, 0 passed, 1 failed. Red TRX: `.artifacts/atlas-instance-design/view-state-red.trx`, SHA256 `c4be8c0150a9b7962f0486040dec843c8bc4464a1378d57a6c3433f9bf1fce57`.

The final normal-build bounded filter `FullyQualifiedName~NativeObserver_NonGui_` executed 21 controls: 21 passed, 0 failed, 0 not-executed. Final TRX: `.artifacts/atlas-instance-design/view-state-final.trx`, SHA256 `db799ebc38fa0cf866419c8f90828fea8243cf0d6be73e4dba8b3ef2ea0741c9`. The actual saved `before` packet contains retained view ID 3 with `IsLoaded:false`, `IsVisible:false`, `TestRetained:true`, and `Attachment:"detached"`. Receipt: `.artifacts/atlas-observer-controls/ca7aea0fdacf447996174e3a50f62701/receipt.json`, SHA256 `a4fae674169ed35dc3cef20ece51ec78fbca4f5cbe789a121c7b30d0668347ef`. This establishes the fields and their observed detached values; it does not claim shown/loaded-true or live native/UIA coverage.

Frozen test source SHA256: `6af54fc651f355fbf22a4b17ecb256007fe6fa4ca8061b382cf61a45eb59841a`. Surface trace: actual per-view public state -> `ViewObservation` -> serialized `Views` JSON -> unshown adapter control -> this receipt. Class -> sweep -> derive -> prevent: a parent object's state cannot prove a retained child's own state; the source and emitted `Views` packet were swept for the omitted fields; the values are derived only from the view reference; the permanent JSON oracle fails when either field is absent. No product path, GUI/native/full suite, private generation/lease reflection, shared register, site, or docs index was changed. Independent Astra review remains the next gate; this author does not self-clear it.
