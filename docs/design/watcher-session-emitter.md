...
summary: >-
  ...
  and pumps them into the store. Also closes the session-end-that-never-ended liveness gap (DC-067).
---
...
`<dataDir>/loomkeeper-coord` (spec US-4 registration; DC-066 residual risk). Nothing in the tool
...
## The liveness gap this closes (DC-067)
...
| Session-end that leaves liveness Alive | Closed by DC-067 — `EndSession` marks the store, liveness reads Ended. |
| Reconcile snapshot read off the UI thread | Safe — `Service.Current` is an immutable layout snapshot. |
| A pane vanishes between ticks | Ended on the next tick (≤2s); residual risk recorded (DC-067). |
