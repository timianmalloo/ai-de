...
DC-042 wired the watcher panes, but they folded **once** on build: a session that registered after a
pane opened stayed invisible until the pane was reopened (DC-042 residual risk; spec US-4 live board,
...
- **Fingerprint** (`WatcherFingerprint`, pure/static): session count + **each session's liveness state**
  ...
  ended session as live (the DC-043 shape, one layer up).
...
- **Gating**: refresh only on a fingerprint change, so an idle watcher never rebuilds a pane — no scroll
  ...
  invalidate (DC-042) so the two paths cannot drift.
