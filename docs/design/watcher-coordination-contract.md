...
- A **live `FileSystemWatcher` tail** that calls `PumpOnce` on change is a thin wrapper over the tested pump, deferred to avoid a DC-040-style flaky FS-watcher surface; the pull-based pump is the tested core.
