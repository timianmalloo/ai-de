...
- **C14(e)(iv) — a categorical refusal set, applied to the RESOLVED path, regardless of the human's pick.** Never attachable: anything under `.git/`, `.ssh/`, `.aws/`, `.azure/`, `.gnupg/`, `.config/gh/`, `.kube/`, or a browser profile directory; any file named `.env*`, `*.pem`, `*.key`, `*.p12`, `*.pfx`, `id_rsa*`, `id_ed25519*`, `.npmrc`, `.netrc`, `.git-credentials`, `credentials`, `*.kdbx`, **`.mcp.json`, `.mcp.json.tmp`** — and, as a second
**class** rather than a longer list of instances, **in-repo config files that carry third-party
credentials in an `env` or settings block**: `.vscode/mcp.json`, `.cursor/mcp.json`,
`docker-compose*.yml`, `*.tfvars`, `local.settings.json`, `.devcontainer/devcontainer.json`,
`appsettings.*.json`. **`.mcp.json` revealed a SECOND CLASS, and that is why the list grew by a class rather than by a
name.** The original list was built from **well-known secret filenames**. `.mcp.json` is not one —
it is an **in-repo config file carrying a third party's credentials in an `env` block**, and the
whole class was invisible to the list's organising idea. Privacy swept it: *"sweep by class, not
instance"*, and was explicit that **the floor language must not be softened by a longer list** —
a longer list is still a floor, and saying otherwise is the security-shaped lie this programme
keeps catching.
**How it was missed is the point.** It holds third-party MCP servers' `env` blocks, which commonly carry **live API keys** — and it sits **inside the workspace root**, so C14(e)(iii) deliberately gives it **no affirmation** either. A file with a live credential was therefore attachable to a send with **no refusal, no label and no prompt**. `.env*` does not match it. `.mcp.json.tmp` is listed beside it because a failed write leaves a byte-complete copy there. *This is not a defect in the clause — it is the floor doing exactly what the clause says it does, and being caught by a second reviewer rather than by itself.* **This list is a floor, not a guarantee** — *"recording it as 'secrets cannot be attached' would be exactly the security-shaped lie this programme keeps catching."* Content scanning is Security's and is deferred. **Oracle:** a table test, one row per entry, asserting zero attachments and one visible refusal naming the rule — **plus one row asserting a near-miss (`env.md`, `keynote.md`) IS attached**, because *a deny-list that also denies the neighbours is a different defect*.
...
describe the caps anywhere as a "data budget"**, and the rule extends: **the C14 caps bound the
ATTACH path only, never what a RUN sends.** The agent's own file reads are unbounded by them —
`GovernedRunHost.Decide` auto-allows every non-edit call. *Fails if:* any UI copy, doc or record
presents 32 KiB / 128 KiB / 5 files as a ceiling on the egress of a run: `paste-to-fence` has no stated cap, so the caps
