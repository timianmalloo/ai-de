...
Fixtures: duplicate IDs; deletion of committed record; syntactically malformed JSON; object missing ID; invalid ID; valid sequential and allocator-minted ULID records; legitimate unique append; reversed valid record order. Each negative must fail for the named finding, not merely throw or return any nonzero. Exercise default logs and explicit paths, missing optional log behavior and real HEAD comparison. These are the fixed cases, not an open-ended permutation sweep. Capture unrelated discovered defects for Owner before changing normal behavior. Real temporary Git repositories preferred to mocks; no global Git config or network. Narrow stdlib changes only.

Executable red-first sequence (Owner decision and independent clearance required): baseline --self-test is NOT a red result because the old CLI accepts it as a missing positional file. First author the intended harness in a disposable copy of the unchanged verifier, keeping production tools unmodified. Run the fixed cases against that baseline. In the fixture copy, inject one mutation at a time: disable duplicate, disappearance, JSON-finding, missing-ID and ID-syntax guards; force always-green CLI; force always-failing CLI. Assert each mutation's target occurs exactly once before applying it, then observe the harness fail with its named discriminating case. The always-failing mutant must be rejected by accepted-shape cases. Preserve measured case/status/diagnostic output in the author Proof Pack. Only after these red observations insert the harness and --self-test routing into the real gate and shrink the one frozen entry in the same commit. Re-run green on the committed candidate; no normal-policy branch changes. The exact harness may be temporarily hosted in ignored fixture files; it must not become a second production validator.
