...
summary: >-
  ...
  DC-062. (A separate finding: the watcher UX is not wired into the running app - see below.)
---

# Investigation: AiDe.App terminal cursor render crash (DC-062)
...
model, must be robust. Registered in `docs/lessons/defect-classes.md` as **DC-062**, controlled by the
mutation-verified regression test.

## 6. Second defect found and fixed (not the crash): the watcher UX was inert at runtime (DC-063)
...
**Fixed** the same day (registered as **DC-063**): a `StartWatcher(dataDirectory)` helper opens the host
...
crash fix (DC-062) was committed first as the urgent stability fix; this wiring fix followed.
