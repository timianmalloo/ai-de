...
| Prompt drafts/revisions and delivery receipts | User-directed composition and dispatch. Context requires explicit attachment and preview. | Owner can view, export, edit, or delete drafts. **Retention has one definition: the 90/365-day ceiling below** (this supersedes the spec's "persist until explicit deletion", which created two contradictory rules for one quantity). Deletion removes drafts/revisions and local attachments; receipt retention is minimum opaque outcome metadata only. |
...
| Agent/user-authored knowledge (`Decision`, `Note`, `Term` via MCP) | 90 / 365 days; default classification `Internal`, unknown→`Restricted` | Daily expiry plus owner deletion; included in the workspace-deletion purge oracle (P1-STORE-DEL). These are authored, not rebuildable from artifacts, so they carry their own retention rather than inheriting the rebuildable-evidence rule. |
...
| DPAPI local snapshots | 7 daily copies / 30 days | Rotation purges expired snapshot and WAL copies. **Recovery boundary:** user-scoped DPAPI blobs are undecryptable after profile loss, local-account password reset, or machine rebuild, so snapshots protect against disk/corruption failure but not profile loss; non-rebuildable prompts/layouts additionally use a **user-passphrase-wrapped export** for cross-machine recovery. A **deletion receipt enumerates surviving snapshot generations and their purge-by date**, and snapshot restore replays the deletion/expiry ledger before serving reads, so a rollback cannot resurrect deleted data. |
...
| **MCP read result → agent session → (possible) external provider** | **Disclosure, Linkability, Non-compliance** | **Indirect model egress: an `ExternalProcessing`/`UnknownProcessing` agent can read workspace facts via `describe`/`find`/`impact` and forward them to its provider. Bind MCP read/write authorization to the target session's processing class as a T0 gateway rule from Phase 1 (ADR-0011): non-`LocalOnly` callers are denied or served minimum non-sensitive metadata; loopback binding is not a sufficient control because the agent is local and its provider is not. Proof: `P1-MCP-EGRESS` denies non-`LocalOnly` reads red-first.** |
