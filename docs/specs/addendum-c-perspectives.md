...
| --- | --- | --- | --- |
| UC1 Agentic Coding — session primary; CLI secondary; hybrid third | **Coding** | today's docking host, unchanged in mechanism | 1st (layout + default), then the mechanism (2nd); the composer conversation (US-C13) is its own item after PR-A/PR-B are filed |
...
| 20 | **Supersessions S-1…S-5 (page one)** | R15 b2 (A:234), the goal-block template row (B:135), B6's form rendering (B:183), R18 b4 (B:209), R19 b2 (B:216) — superseded by the operator's verdicts (2) and (3). | **File as two numbered rulings — PR-A (session-level settings: fan-out cap · budget, and the question whether tier joins them) and PR-B (the composer is a conversation; templates render as inline derived structure; S-8's round-trip extension) — at the next free numbers.** The decision is the operator's for S-1…S-5; tier and S-8 are this document's inferences for the Owner to confirm; the filing is the formality; this document cites no number until the note exists. |
