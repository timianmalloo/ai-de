...
- **K0 contract grounding:** `proof-code-atlas-contract-grounding` records source-only findings and worker-reported targeted executions. The report and corrections are joined through `e4229845`; its command/TRX and synthetic-output records are the evidence sources **[Reported execution; observed report content, not an independent rerun]**. K0 does **not** admit its earlier type-only minimal slice; the Conductor rejected that slice as incompatible with the Owner-required physical inventory and addressable-member journey.
