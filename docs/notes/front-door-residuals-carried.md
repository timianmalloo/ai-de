...
| **DC-095 is controlled in code comments and uncontrolled in Proof Packs** | measured | `verify-cited-controls.py` scans `SEARCHED = ("src", "tests")` — **source comments only**. A Proof Pack is markdown, so every control it cites is unguarded, in the artifact whose entire purpose is citing controls. Found by resolving all **14** test-shaped identifiers in `conductor-front-door.md` against `tests/`: **one did not exist** — the header's falsifier suite named `TheRunHasOneCompositionRootTests`, a class that has never existed (the real one is `TheOneCompositionRootIsCountedTests`). Corrected at this close. **The gate extension is named and not built:** the resolver already exists twice, as `_declared_in_tests` in the F5 oracle and as the matcher in `verify-cited-controls.py`. |
