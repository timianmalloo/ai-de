...
## Ruling 121 — Codex's E1 and E2 contract spikes granted as branch-local authoring of exactly the named files; not in `AiDe.sln`, compiled by the coverage gate, admitting nothing

*Filed by the conductor verbatim from the Owner's return (2026-09-15, 20:35Z). Prompted by Codex's `req-01M2KBCP7JY9PJTC63PD04WPT2` (E1: `spikes/atlas-behavior-contract/{AtlasBehaviorContractSpike.csproj,Program.cs,RESULT.md}` + `docs/proof/atlas-behavior-contract.md` — "can exact selected-method occurrence/control-flow facts preserve repeats, overload/recursive identity, branches/async/throw and unknown dispatch without inventing runtime order?"; console Main only, the centrally pinned Roslyn 4.14.0, synthetic source parsed never executed) and `req-01M2KBCP9A8893CWVDDGKKTBYS` (E2: `spikes/atlas-architecture-contract/{AtlasArchitectureContractSpike.csproj,Program.cs,architecture.fixture.json,RESULT.md}` + `docs/proof/atlas-architecture-contract.md` — a proposed domain/layer JSON carrier against a synthetic identity set and source-as-data resource identity/equality; stdlib only). Both are the Spike Protocol's shape: a contract question answered by an executed spike before any design admission. The Owner verified §2's `spikes/**` row, `AiDe.sln:30–46`, the 20 existing spike projects (16 outside the solution), `Directory.Packages.props:26` pinning `Microsoft.CodeAnalysis.CSharp` 4.14.0, and `verify-project-coverage.py:15–19` — the gate compiles every tracked project outside `AiDe.sln` on every push, so a spike is never "kept out" of the build graph.*

**RULING:** Grant both spikes — E1 and E2 — as branch-local authoring of exactly the named files, conditions (a)–(e) as the conductor proposed with (a) amended below; the grants lapse at landing or the end of the stress test.

**BECAUSE:** `spikes/**` is Core's (§2), the directories do not exist, and no lane, lease or seam touches them; the Spike Protocol is the pack's required move before depending on a contract, and both requests ask a real contract question with negative fixtures first. The build-graph fact is verified and cuts the other way from the conductor's worry: the coverage gate exists precisely so that no tracked project escapes compilation (`verify-project-coverage.py:15–19`), so the spike is inside the build graph by design.

**CONFIDENCE:** Verified (§2 row, `AiDe.sln:30–46`, the 20 existing spike projects, `Directory.Packages.props:26`, the coverage gate's docstring); Inferred (the spikes' code — not yet written).

**SCOPE EFFECT:** (a) amended: the two `.csproj` files are **not** added to `AiDe.sln` (the solution answers "what ships"; 16 of 20 spikes are outside it), and they **must compile clean under `verify-project-coverage.py`** on Windows and Linux runner shapes, since the gate will build them on every push; the gate's printed wall time is recorded before and after in the landing receipt (a measured ring decision, not a comment). E1 references only the centrally pinned Roslyn; E2 references nothing beyond the framework. (b)–(e) stand: synthetic input only, never executed, no live repo data, no desktop; `RESULT.md` records observed outputs, negative fixtures first, and names what the spike could not answer; the conclusion admits nothing — `SelectBehaviorAsync`, `ReadAsync`, any surface row, come back as their own requests after the Atlas foundation lands; lands under 108 as docs+spikes with an empty enumerated failing set, joined by the Claude conductor after Atlas.

**CONDITIONS:** (i) No `src/`, `tests/`, production signature or `Directory.Packages.props` change — a spike that needs a package-version change stops and files a request. (ii) The `docs/proof/*.md` files carry V2 frontmatter and the observed run's command and output, so a reader not in the session can re-run and dispute. (iii) A spike that does not build on the Linux coverage run is not landed — the fast ring is not to be reddened by evidence artifacts. (iv) The conductor resolves both requests citing this ruling.

**RECORD AS:** Ruling 121 — Codex E1/E2 contract spikes granted (exact files, synthetic never-executed input, negative-first, RESULT.md observed); not in AiDe.sln, compiled by verify-project-coverage on every push, wall time recorded; the spike admits no design.


---

## Ruling 122 — `DeclaredDeploymentContext` is admitted as a seam to be specified, not built; the data-model floor and the D&P Architect's review stand between admission and implementation

*Returned by the Owner 2026-09-15 20:55Z; **filed unchanged on 2026-09-16** after the Claude session's usage cap — the text is the Owner's as returned, with one dated rider below.*

*Filed by the conductor verbatim from the Owner's return (2026-09-15, 20:55Z). Prompted by Codex's `req-01M2KC9CR6YCH2NRZRYW28JFT3` after its E2 spike (`55df5b6e`): "source literal targetScope identifies only scope kind; no actual subscription/resource-group context. Full US-E8.b cross-declaration equality stays open. Please identify or admit a read-only DeclaredDeploymentContext producer bound to exact template/declaration manifest/hash, explicit deployment-scope identity and full parent/type/name chain, per-value provenance and parameter bindings, typed unresolved for missing/conflicting/expression values … Label declared configuration identity, never observed deployment. No cloud calls, credentials, execution, defaults-as-fact, new implementation permission or live Azure requested." Verified by the conductor and the Owner: nothing named `DeploymentContext` / `targetScope` / `BicepDeclaration` / `IDeclaredDeployment` exists under `src/`; `spikes/bicep-as-data/RESULT.md:29–44` (2026-08-28) fixed the principle one level down — "16 of 24 resource names are literals. Eight are expressions … They are kept **verbatim** and disclosed as expressions rather than guessed. A join that needs a literal resource name is therefore `Inferred` at best, and must say so. A guessed name would produce a confident wrong edge between a table and a server, which is worse than an absent one: the user would act on it." — with the recorded upgrade trigger at `:44`: "upgrade trigger = a join requires a resolved expression."*

**RULING:** (a)–(e) as the conductor proposed, confirmed: there is no existing producer to identify; a read-only `DeclaredDeploymentContext` producer is admitted into Codex's E2 design as a Core contract *proposal* with the fixed properties as listed (exact template/declaration manifest and content hash; explicit scope identity as scope kind plus declared value or typed-unresolved; full parent/type/name chain; per-value provenance to a source span; parameter bindings; typed unresolved for missing, conflicting and expression values — never defaulted, never evaluated; labelled declared configuration identity, never observed deployment); implementation stays ungranted until the Atlas foundation lands, the D&P Architect has reviewed the aggregate, and the seam's home is ruled in a second request; US-E8.b stays open and is named open.

**BECAUSE:** `bicep-as-data/RESULT.md:32–34` already fixed the principle for one level (names as expressions kept verbatim, joins on them Inferred at best) and `:44` recorded the upgrade trigger as "a join requires a resolved expression" — Codex's spike (`55df5b6e`, their report) is that trigger firing for scope, and the correct response to a trigger is a specified contract, not an inferred value. The pack makes the data model the highest-priority decision with the D&P veto (DM1–DM18); a producer whose output is an identity with a history rule is exactly the artifact that veto exists for.

**CONFIDENCE:** Verified (no producer in `src/`; the bicep-as-data principle and its trigger); Inferred (the E2 spike's finding that `targetScope` carries only scope kind — Codex's observation, not opened).

**SCOPE EFFECT:** Admits the seam as a design item in Codex's E2 programme docs. Fixes two more properties: (f) the aggregate is the *declaration* — one row is exactly one declared resource within one template hash — and equality across declarations is a derived, evidenced relation, never a stored merge (DM "derive, don't store"); (g) "typed unresolved" is a closed vocabulary (missing · conflicting · expression) with the source span, so an absent value degrades to "not recorded", never to a default. Cuts: no cloud, credentials, execution, live Azure, no new store or graph, and no change to the existing Bicep extractor — the producer *reads* what it already yields.

**CONDITIONS:** (i) Codex's E2 design names the bounded context, the aggregate and its invariant, the grain sentence, each attribute's history rule, and which quantities are derived — before the D&P review is requested. (ii) The D&P Architect's review is convened by the Claude conductor on a `request-add` from Codex; its outcome (accept, amend, veto) is the gate to an implementation request — that request also proposes the home (`Core/Projections/` vs a new context) and the Owner rules on it then. (iii) The spike's negative fixtures are carried into the design as the contract's first named tests, with their source anchors. (iv) The conductor resolves `req-01M2KC9CR6YCH2NRZRYW28JFT3` citing this ruling and `bicep-as-data/RESULT.md:29–44` verbatim.

**RECORD AS:** Ruling 122 — DeclaredDeploymentContext admitted as a specified read-only seam (declared-never-observed, typed-unresolved, per-value provenance, declaration as aggregate); D&P Architect review gates implementation; home ruled on a second request; US-E8.b named open.

**RIDER (Owner, 2026-09-16):** E2 design work Codex produced against the seam during the 22-hour gap, and the cleared E2 spike, are design serialization and spike evidence only — neither advances the seam past "specified", and the D&P Architect review still gates the first implementation file. Nothing that happened in the gap was an input this ruling depended on (Ruling 121 already fixed that a spike admits no design, and the D&P gate was this ruling's own condition); filing it a day late changes the filing note, not the decision.


---

## Ruling 123 — the duplicate `Type` keyword is the grammar's defect; the Color conflict is not a defect; `x-` extension admitted for Codex's serialization only; the source fix is a Claude knowledge lane

*Filed by the conductor verbatim from the Owner's return (2026-09-16). Prompted by Codex's `req-01M2KD0HNAYKZPBRW2SM9PR63N`: "examples duplicate Type facet for category and typography, and Color:HighContrast+DarkAdaptive conflicts with one-Color/one-facet grammar … Please route source grammar/example correction to its owner … Independent UX review must verify normalization preserves intent." Both the conductor and the Owner opened the grammar rather than taking the report: the facet table carries `Type` twice — `.claude/knowledge/ui-archetype-grammar.md:160` "The product archetype family (selects the catalog row), *required*" and `:168` "`Type` (Typography) — Type personality (families live in tokens), `Utilitarian`" — while `:66` requires each facet at most once; and no signature anywhere in the repo writes `Color:HighContrast+DarkAdaptive` (grep over `docs/specs/*.md` returns none; Addendum C §C1 uses `Color:DarkAdaptive` alone).*

**RULING:** (a) The duplicate-`Type` finding is the grammar's, confirmed on the file: `Category ::= "Type" ":"` (`:73`) and `Typography ::= "Type" ":"` (`:110`) share one keyword while `:66` requires "each facet at most once" and G5 (`:148`) lists `Type` twice; every canonical signature (catalog ×27, `DESIGN.md:4`, `docs/specs/addendum-c-perspectives.md:1286`) violates the grammar's own rule. (b) The `Color:HighContrast+DarkAdaptive` conflict is **not** a grammar contradiction: no signature in the repo writes it; it is a single-valued facet meeting a two-valued need, which is G15's stated case for `x-`. (c) Codex's `Type:DSS; x-typography:MonospaceTechnical; Color:DarkAdaptive; x-highContrast:System` is admitted for the five-view design serialization only, marked in the design as a G15 extension, conferring nothing on the grammar and clearing no gate. (d) The source correction is pack knowledge and lands in a Claude lane: give Typography its own keyword, fix G5's list and the §3 table, update the catalog's 27 canonical signatures and both installed copies (`.claude/knowledge/`, `.github/knowledge/`) together per G16, and add one reading rule so existing product signatures written with two `Type:` keys stay valid without being rewritten. (e) One defect-class entry: a control's vocabulary that admits two readings of one token.

**BECAUSE:** The Owner opened the grammar and the examples rather than taking either lane's word; the duplicate key is real and the Color conflict is absent from every example, so Codex found one defect and one limitation. No pack script parses signatures (verified), so the fix is documentation-only and smallest-correct is a keyword, not a parser.

**CONFIDENCE:** Verified (grammar lines 66/73/110/148/160/168; catalog and spec signatures; absence of a parser and of any `HighContrast+DarkAdaptive` example).

**SCOPE EFFECT:** Admits the Claude knowledge-doc lane in (d); cuts any rewrite of `DESIGN.md`, Addendum C or understanding-views signatures in that lane (the reading rule covers them). Whether Windows high-contrast is a `Color` flavor or an `A11y` obligation is the UX & Accessibility lens's call under G6 in its review of Codex's design, not the Owner's.

**CONDITIONS:** (i) The lane fixes the file at the pack's source of truth so `/updatepack` does not revert it, and records where that is. (ii) The UX & Accessibility lens verifies Codex's normalization preserves intent, as Codex asked. (iii) The conductor resolves `req-01M2KD0HNAYKZPBRW2SM9PR63N` citing this ruling and stating (b) plainly so Codex does not wait on a Color repair that is not coming.

**RECORD AS:** Ruling 123 — duplicate `Type` keyword is a verified grammar defect (Claude knowledge lane: keyword + catalog + both copies + reading rule); `Color:HighContrast+DarkAdaptive` is a G15 case, not a defect; Codex's `x-` facets admitted for design serialization only; one defect-class entry.

---

## Ruling 124 — Ruling 121's spike conditions met on the evidence, less two checks; combined coverage run required; the spikes land independent of Atlas

*Filed by the conductor verbatim from the Owner's return (2026-09-16). Prompted by Codex's `req-01M2KD3D85NBQXEN2BWPZQTAWE`: "Frozen E1 5d361f2a…, repaired E2 90189411… independently cleared by acdf4894… Conductor 7c967528 proof records manual Linux SDK 10.0.303: E1 build 3.49 s / 6 groups + 3 negative, E2 build 1.91 s / 28 checks; each Windows project-coverage 40 tracked / 18 solution / 22 outside / 0 exempt, 26 s vs common 39/18/21 baseline 26 s. Combined candidate coverage not yet run; landing after Atlas under 108 remains yours."*

**RULING:** Ruling 121's conditions are met on the evidence given for (a) wall time (26 s before, 26 s after, each spike alone) and (b)–(e) (synthetic, negative-first, `RESULT.md`, admits nothing), with the manual Linux SDK 10.0.303 builds accepted as the Linux evidence because the coverage gate does not run on Linux today (`build.yml:649` is inside `build`, `windows-latest`). Before the landing intent: (1) one **combined** `verify-project-coverage.py` run with both spikes present, expected 41 tracked / 18 solution / 23 outside / 0 exempt, wall time recorded; (2) the conductor opens both `docs/proof/*.md` and confirms V2 frontmatter plus the observed command and output (121(ii)) — a report that they exist is not that check. Amend 121's "after Atlas": the spikes touch only `spikes/**` and `docs/proof/**`, no Atlas seam, so they land under 108 with an empty enumerated failing set as soon as (1) and (2) are recorded, not behind the Atlas landing.

**BECAUSE:** The ordering in 121 was a convenience while Atlas was imminent; it has been 22 hours with no landing and no forecast, and holding docs-plus-spikes behind a stalled candidate buys nothing. The counts add up (baseline 39/18/21, each spike +1 → 40/18/22), so the combined run is a confirmation, not a hope.

**CONFIDENCE:** Verified (build.yml coverage placement; Ruling 121 as filed); Inferred (the build timings, counts and the independent clearing — Codex's and the conductor's reports, not opened by the Owner).

**SCOPE EFFECT:** Frees the spike landing from the Atlas ordering; nothing else.

**CONDITIONS:** (i) The landing intent names both proof docs' frontmatter check as observed. (ii) Ruling 121(i) still holds — no `src/`, `tests/`, or `Directory.Packages.props` hunk. (iii) The conductor resolves `req-01M2KD3D85NBQXEN2BWPZQTAWE` citing this ruling.

**RECORD AS:** Ruling 124 — Ruling 121 checkpoint met (manual Linux builds accepted, 26 s unchanged); combined coverage run and proof-doc frontmatter check required before the intent; spikes land under 108 independent of Atlas.

---

## Ruling 125 — the five dropped controls: four admitted as one Claude lane, one deferred to a measurement; the audit-log fix sequences after Codex's Ruling 119 commit

*Filed by the conductor verbatim from the Owner's return (2026-09-16). The five were recorded as findings on 2026-09-15 and none was converted while the Claude session was capped: `coord claim` still accepts leases on append-only registers and derived views (four collisions that day, one of which stopped a `main` join at its commit step); `docs-graph validate` is absent from the join's gate line (a join passed 38/38 over a broken link graph); `verify-audit-log` accepts an absent log — including a tracked one gone missing — and passes a non-object JSON line to `entry.get` (Codex's findings, kept out of its Ruling 119 scope); `verify-project-coverage` runs only in `build.yml`'s windows job; and the unidentified cross-tree "rescue stranded audit" sweeper has no register entry.*

**RULING:** Admit as one Claude lane: (i) `coord claim` refuses a path whose registry class resolves to `register` or `derived` — reuse `load_registry` (`coord-core.py:833`) beside the existing `COORD-CLAIM-SELF` refusal (`:143–147`), its own code and self-test; an unregistered repo stays advisory as today. (ii) `docs-graph validate` in the gate line, in whatever shape the pack runner enumerates gates (the repo file only passes `--args`, line 33) — the lane runs validate **first**, records the state, repairs the link graph in the same lane if red, then wires the gate, so no join is reddened by a control that was true before it. (iii) `verify-audit-log.py`: a `git ls-files`-tracked absent log is a finding, an untracked absent one stays "(absent)"; a decoded line that is not an object is a finding ("not an object") and continues; both as self-test cases — this is the Ruling 119 policy request, granted. (v) Three defect-class entries: the unidentified cross-tree sweeper; leases on append-only/derived artifacts; the grammar token with two readings (Ruling 123(e)). **Defer** (iv): the lane runs `verify-project-coverage.py` once on Linux locally (WSL or container), records which of the 22 outside-solution projects build there, and files that measurement with the SRE and Test Architect — no `build.yml` change and no muted step until then.

**BECAUSE:** (i) is the control that stopped a `main` join; (ii) let a join pass 38/38 over a broken graph; (iii)'s two branches are open on the file (`:136–140`, `:154`); all three are small, stdlib, and have a ready self-test shape. (iv) is not a free move: the gate has only ever run on Windows, and an outside-solution project targeting `net10.0-windows` would redden the fast ring the day it is added — measure before wiring.

**CONFIDENCE:** Verified (coord-core refusal and registry, the verifier's two branches, both gate runners, build.yml job placement).

**SCOPE EFFECT:** Admits (i)(ii)(iii)(v); defers (iv) to a measured request; cuts any `build.yml` edit from this lane. `coord-core.py` and the pack runner are pack scripts, not `tools/**` — the lane checks §2 for their row and fixes at the pack's source of truth.

**CONDITIONS:** (a) (iii) lands **after** Codex's Ruling 119 commit on `tools/verify-audit-log.py`, which holds a live branch-local grant on that file; Codex's 119 commit may land now (docs/tools, no Atlas seam) and the Claude cases go into the `--self-test` it adds. (b) Red-first evidence per planted case in the receipt. (c) Each control its own commit; the defect-class entries cite the instance date and the control that now fails on recurrence.

**RECORD AS:** Ruling 125 — dropped controls: claim refusal on register/derived classes, docs-graph validate in the gate line (repair then wire), verify-audit-log tracked-absent and non-object findings (after Codex's 119 commit), three defect-class entries admitted; Linux coverage deferred to a measured request.

---

## Ruling 126 — `lane/main-red-0915` opens now for groups 1–2; groups 3–4 held with a 48-hour termination variant; seam notice before the first edit

*Filed by the conductor verbatim from the Owner's return (2026-09-16). Ruling 112(2) admitted the lane and Ruling 117 ordered its groups "after the Atlas landing"; the premise failed. Measured by the conductor: `main` last advanced 2026-09-15 22:39Z, the Atlas accepted integration has not landed in 22 hours with its P1-03 qualification failing, `main` has been red since 2026-09-12 (issue #13), and the red set grew from 13 to 15 at `bcf4959b` (run 35056077857) — the nine deterministic members bit-identical to INV-0012's, the intermittent population rotated to six `Sessions.Thread.TheThreadIsChatLikeTests`.*

**RULING:** Open `lane/main-red-0915` now for Ruling 117's groups 1 and 2 only: `Platform=Windows` traits on the four `EngineCatalogTests` locator tests and the purge test, plus the one portable characterisation test — put it in a **new file**, not inside the two existing ones, to keep the edited hunks to attribute lines. Hold groups 3–4 (the WPF extent tests, the intermittent App population) until the Atlas accepted integration lands **or 2026-09-18 21:00Z, whichever is first**; if the deadline fires, the conductor brings groups 3–4 back to the Owner with Atlas's status — the cap is a defect signal, not a ruling to proceed. Ruling 118's reference-census gate may ride in this lane as its own commit.

**BECAUSE:** Ruling 117's "after the Atlas landing" was premised on an imminent landing; the premise failed (22 hours, P1-03 failing, no forecast) while the red set grew 13 → 15 with the nine deterministic reds bit-identical to INV-0012's. Groups 1–2 are trait-only edits to two Core test files, the smallest collision surface the lane has; groups 3–4 are the App assemblies Atlas is qualifying now, so they stay held.

**CONFIDENCE:** Verified (Ruling 117 as filed; the group scope); Inferred (the 15-red set and the 9 bit-identical members — the conductor's `.trx` reading; that the Atlas candidate *modifies* rather than merely carries the three files — not shown to the Owner).

**SCOPE EFFECT:** Admits groups 1–2 and the 118 gate now; freezes groups 3–4 under the dated hold; nothing outside those tests plus the two new files is touched (117(iii) stands).
