...
**Status counts:** controlled 99 · partially-controlled 65 · uncontrolled 20
...
### DC-182 — A count cited in a Proof Pack or a comment is a memoir; only the literal's own count is a record
...
### DC-183 — An oracle over what did not happen passes a run that never ended
...
### DC-184 — A poll-after-event drain waits on an empty queue when the producer publishes the terminal event before resolving the request
