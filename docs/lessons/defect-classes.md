...
**Status counts:** controlled 82 · partially-controlled 59 · uncontrolled 17
...
### DC-158 — A container's disabled-state trigger only reaches ink that inherits it, and a template that sets ink locally, or carries no trigger at all, is invisible to a floor that never renders the state
