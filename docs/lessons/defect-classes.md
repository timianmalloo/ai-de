...
**Status counts:** controlled 69 · partially-controlled 55 · uncontrolled 15
...
### DC-137 — The leaf overrides the container's pairing: an implicit style on a text leaf outranks every state ink the container sets by inheritance
...
### DC-138 — A UI defect report is evidence about a BINARY, and the binary is never named, so a fixed instance re-enters as a recurrence
...
### DC-139 — A scripted edit anchors on the first occurrence of a token that prose also contains, and the tree it produces still parses
