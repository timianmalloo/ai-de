...
### DC-230 — A fixed extent on a region whose whole job is to fill its parent, written in CSS inside a string literal where no gate can see it

- **Shape:** a container is given a constant size in a medium none of the project's controls inspect; every downstream measurement then honours it faithfully, so the content is *correctly* filling a wrongly-small box and the defect reads as a content problem.
- **Signature:** a pane whose chrome (splitter, header, footer) spans the full height while its primary content stops short; scaling arithmetic that predicts the drawn extent to the pixel; the constant appearing nowhere in any test.
- **Instance (2026-09-16):** `CanvasPage.cs:65` — `#stage { height: 440px }` inside a full-height Explore pane. Measured 1624×437 stage in a 1661×2002 pane: **21.8% of the height, 3.6% of the pane's area, ~72 px² per drawn node**. `fit()`'s own arithmetic (437 − 100 + 18 = 355) predicts the measured 354 px disc, which is what proves the container rather than the content is at fault.
...
- **Control:** a headless contract test over every inlined page string asserting that the region which is the pane's primary content declares **no absolute `px` height** and **does** declare a flex remainder with a `min-height`. Deterministic, no window, no WebView2. Seen failing first on `height: 440px`.
- **Status:** `uncontrolled` — `INV-0014` P0/P1; state a frame-time budget as an acceptance criterion on the fix, since the stage grows ~20× in area and the canvas has never been measured at any size.
