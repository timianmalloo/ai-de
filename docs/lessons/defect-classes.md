...
### DC-nnn — A scheduled rename lists the declaring file, and its reference set is discovered by the build inside the node
...
### DC-nnn — A probe prints a type's default `ToString()` across a process boundary and a test asserts the string
...
### DC-nnn — A Proof Pack figure or "red observed" cell is written before the measurement that would fill it
