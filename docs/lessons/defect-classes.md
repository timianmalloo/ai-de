...
**Status counts:** controlled 20 · partially-controlled 27 · uncontrolled 0
...
- **Residual risk:** the double encoding is still there; the factor manages it rather than removing it. The real fix is an envelope carrying raw JSON, at which point the two sizes agree and both the factor and `FramedCost` disappear — recorded as the `simplify:` upgrade trigger on `MaxResponseBytes`. Separately, shrinking is not monotonic: asking for 5,000 nodes can return fewer than asking for 1,500, because the larger request overshoots on the way down.
- **Status:** `partially-controlled`
